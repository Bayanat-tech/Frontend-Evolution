...
export async function getLevel5ApprovalDetails(acCode: string) {
  const response = await api.get<ApiResponse<Record<string, unknown>>>(
    `/api/finance/ac-tree/level5/${encodeURIComponent(acCode)}/approval`
  );
  if (!response.data.success) throw new Error(response.data.message || "Unable to load approval details");
  return response.data.data || {};
}

export async function saveLevel5ApprovalDetails(acCode: string, payload: {
  cr_no?: string;
  ac_status?: string;
  ac_active?: string;
  approved_by?: string;
  ...
  if (!response.data.success) throw new Error(response.data.message || "Unable to save approval details");
  return response.data;
}

export async function getLevel5Activities(acCode: string) {
  const response = await api.get<ApiResponse<Record<string, unknown>[]>>(
    `/api/finance/ac-tree/level5/${encodeURIComponent(acCode)}/activities`
  );
...
export async function upsertLevel5Activities(payload: {
  ...
  const response = await api.post<ApiResponse<null>>(
    `/api/finance/ac-tree/upsertVendorActivity`,
    payload
  );
...
export async function upsertLevel5Documents(payload: {
  ...
  const response = await api.post<ApiResponse<null>>(
    `/api/finance/ac-tree/upsertAcMasterDocsDet`,
    payload
  );
