...
import type { FinanceReportDocument } from "./financeReportPreviewStore";

export type ReportIdentity = { title: string; company: string; user: string; generatedAt: string };
export const reportFilename = (title: string) => title.replace(/[<>:"/\\|?*\x00-\x1f]/g, "-").trim() || "Finance report";
...
export async function buildFinancePdfDefinition(report: FinanceReportDocument, identity: ReportIdentity, browserWindow: Window = window): Promise<TDocumentDefinitions> {
  ...
  return {
    info: { title: identity.title, author: identity.company, subject: "Finance report" },
...
export async function createFinancePdf(report: FinanceReportDocument, identity: ReportIdentity) {
  const [{ default: pdfMake }, definition, vfs] = await Promise.all([
    import("pdfmake/build/pdfmake"), buildFinancePdfDefinition(report, identity), fontVfs(),
  ]);
...
export async function downloadFinanceExcel(report: FinanceReportDocument, identity: ReportIdentity) {
