...
import { Ban, Building2, ChevronDown, ChevronUp, Edit2, FileText, List, Paperclip, Plus, Printer, Receipt, RefreshCw, Save, Search, Trash2, X } from "lucide-react";
  ...
  const updateDetail = (id: string, patch: Partial<TransactionDetail>) => {
   ...
   const updateDetailAmount = (id: string, newAmount: number) => {
    setForm((current) => {
      ...
      return {
        ...current,
        // âœ… Keep user's typed amount, don't snap it
        detail: current.detail.map((row) =>
          row.id === id ? { ...row, amount: newAmount } : row
        ),
            ...
            {!showHeaderDetails ? (
              <div className="flex items-center justify-between px-3.5 py-1.5 bg-blue-50/70 border border-blue-200 rounded-lg text-xs shadow-xs">
                <div className="flex items-center gap-4 text-slate-700 flex-wrap min-w-0">
                  ...
                  {form.ac_payee && (
                    <span className="inline-flex items-center gap-1.5">
                      <span className="font-semibold text-[#00378C]">Payee:</span>
                      <span className="font-medium">{form.ac_payee}</span>
                    </span>
                  )}
                </div>
                <Button
                ...
                <div className="grid grid-cols-2 gap-2.5 max-xl:grid-cols-1">
                  ...
                  <div className="finance-payment-header-block">
                    ...
                    <div className="finance-payment-header-fields">
                      ...
                      {docType === "CP" && (
                        <Field label="Account Payee">
                          <Input disabled={disabled} value={form.ac_payee || ""} onChange={(event) => updateField("ac_payee", event.target.value)} />
                        </Field>
                      )}
                      <div className="col-span-2 max-md:col-span-1">
                        <Field label="Remarks">
                ...
                <table className="finance-lines-table w-full min-w-[2140px] text-xs">
                  ...
                  <tbody>
                    {form.detail.length === 0 ? (
                      <tr><td className="px-3 py-8 text-center text-muted-foreground" colSpan={17}>No detail lines yet — click "Add Line" to get started</td></tr>
                    ) : filteredDetail.length === 0 ? (
                      <tr><td className="px-3 py-8 text-center text-muted-foreground" colSpan={17}>No lines match "<strong>{lineSearch}</strong>"</td></tr>
                      ...
                      <>
                        ...
                        {expandedRowIds[detail.id] && (
                          <tr key={`${detail.id}_alloc`} className="bg-slate-50/70 border-b border-blue-200/60">
                            <td colSpan={17} className="p-0 pl-10 pr-3 pb-2 pt-0.5">
