...
function PaymentDocumentEditor({
  ...
  const selectDetailAccount = async (detail: TransactionDetail, value: string, row: LookupRow | null) => {
    updateDetail(detail.id, { ac_code: value, ac_name: text(getLookupValue(row || {}, "ac_name")), child_table: "", child_code: "" });
    if (!value) return;
  ...
  const updateChildRow = (childId: string, patch: Partial<TransactionChildRow>) => {
    ...
    setForm((current) => {
      ...
      return { ...current, children: { ...current.children, [selectedDetail.id]: rows } };
    });
  };
        ...
        {loading ? (
          ...
          <div className="grid gap-3">
            ...
            <div className="payment-header-grid grid grid-cols-6 gap-2.5 rounded-md border bg-card p-3 max-2xl:grid-cols-4 max-xl:grid-cols-3 max-lg:grid-cols-2 max-md:grid-cols-1">
              ...
              <Field label="Doc Date"><Input disabled={disabled} type="date" value={dateInput(form.doc_date)} onChange={(event) => updateField("doc_date", event.target.value)} /></Field>
              ...
              {docType !== "CR" && <Field label="Cheque Date"><Input disabled={disabled} type="date" value={dateInput(form.cheque_date)} onChange={(event) => updateField("cheque_date", event.target.value)} /></Field>}
                  ...
                  <tbody>
                    {form.detail.length === 0 ? (
                      ...
                      <tr className={selectedDetail?.id === detail.id ? "border-t bg-primary/5" : "border-t odd:bg-muted/20"} key={detail.id}>
                        ...
                        <td className="w-32 px-2 py-1"><Input disabled={disabled} type="number" step="0.001" value={detail.amount} onChange={(event) => updateDetail(detail.id, { amount: Number(event.target.value || 0) })} /></td>
                        ...
                        <td className="w-32 px-2 py-1"><Input disabled={disabled} value={detail.tx_compntcat_code_1 || ""} onChange={(event) => updateDetail(detail.id, { tx_compntcat_code_1: event.target.value })} /></td>
                        <td className="w-28 px-2 py-1">
                          <Select disabled={disabled} value={detail.tx_compnt_1_expmt || "N"} onChange={(event) => updateDetail(detail.id, { tx_compnt_1_expmt: event.target.value })}>
            ...
            <div className="rounded-md border bg-card">
              <div className="flex flex-wrap items-center justify-between gap-2 border-b bg-secondary/40 px-3 py-1.5">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button disabled={disabled || !selectedDetail?.child_table || childLoading} size="sm" type="button" variant="outline" onClick={() => selectedDetail && void loadChildrenForDetail(selectedDetail)}>
          ...
          {loading ? (
            ...
            <tr className="border-t" key={row.id}>
              ...
              {childTable === "invoice" ? (
                ...
                <>
                  <td className="px-2 py-1"><Input disabled={disabled} value={text(row.exp_subtype_code)} onChange={(event) => onChange(row.id, { exp_subtype_code: event.target.value })} /></td>
                  <td className="px-2 py-1"><Input disabled={disabled} value={text(row.exp_code)} onChange={(event) => onChange(row.id, { exp_code: event.target.value })} /></td>
              ...
              {childTable === "invoice" && <td className="w-32 px-2 py-1"><Input disabled value={formatNumber(Number(row.c_bal_amt_org || 0) - Number(row.amount || 0))} /></td>}
