...
// Generic shape returned by all HR_CODE_VALUES-backed lookups
// (Ticket Eligible / Medical Eligibility, Ticket Type, Blood Group, Status).
...
export function HrEmpDependantsPage() {
    ...
    // ── Master data — HR_CODE_VALUES driven dropdowns ───────────────────────────
    // GROUP_CODE = 0  -> Ticket Eligible / Medical Eligibility (shared Y/N list)
    ...
    const mutation = useMutation({
        mutationFn: async () => {
            ...
            // FRONTEND-ONLY FIX: the backend requires DEP_SERIAL_NUMBER on every
            // row, and PROC_INS_UPD_HR_EMPLOYEE_DEP fully deletes + reinserts
            // this employee's dependants on every save. So the serial number
            // only needs to be unique *within this save batch* — we regenerate
            // it here (1-based, by current row order) instead of relying on
            // whatever was in dep_serial_number (which is "" for new rows and
            // was otherwise sent as null, causing the backend's required check
            // to throw "DEP_SERIAL_NUMBER required at row N").
            const dependant_details = rows.map((r, idx) => ({
        ...
        onSuccess: () => {
            ...
            // Keep local rows in sync with the serial numbers that were just
            // sent to (and stored by) the backend, so a subsequent edit
            // without a refetch still lines up.
            setRows((prev) =>
                prev
                    .filter((r) => r.status_flag !== "D")
                    .map((r, idx) => ({ ...r, _isPersisted: true, dep_serial_number: String(idx + 1) })),
            );
    ...
    return (
        <section className="grid gap-4">
                ...
                <div className="flex flex-wrap items-center gap-2">
                    <Button
                        ...
                        onClick={() => {
                            ...
                            void queryClient.invalidateQueries({ queryKey: ["hr-dependants-eligible", companyCode] });
                            void queryClient.invalidateQueries({ queryKey: ["hr-dependants-ticket-type", companyCode] });
                            void queryClient.invalidateQueries({ queryKey: ["hr-dependants-blood-group", companyCode] });
            ...
            <DataTable
                ...
                searchPlaceholder="Search name, relation..."
                height={420}
                minWidth={2600}
                density="grid"
