...
const AddPayUnitsForm = ({ onClose, isEdit, isViewMode = false, pay_comp_id, div_code, div_name }: TProps) => {
  ...
  const formik = useFormik<TPayUnitFormValues>({
    ...
    enableReinitialize: false, // We control values manually via setValues
  ...
  useEffect(() => {
    ...
    // Only pre-fill div from props when ADDING (not editing)
    if (!isEditOrView) {
  ...
  useEffect(() => {
    ...
    formik.setValues({
      ...getInitialValues(),
      ...normalized,
      // Keep company_code from auth if API doesn't return it
      company_code: normalized.company_code || user?.company_code || '',
      detail: formik.values.detail // preserve existing detail rows
    });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  return (
    <div className="w-full">
      ...
      {/* Show spinner while loading for edit/view */}
      {isLoadingData && (
