...
function getStatusMeta(
  ...
  const isDraft =
    ...
    (
      status === "DRAFT" ||
      lastAction === "SAVE AS DRAFT" ||
      lastAction === "SAVE_AS_DRAFT"
    );
...
function computeFinalRating(
  ...
  if (weightage?.isHrDefined) {
    return Math.round(
      taskScore * (weightage.taskPct / 100) +
      charScore * (weightage.charPct / 100)
    );
  }
...
const MyTaskPage = ({ initialTab = 0 }: MyTaskPageProps) => {
  ...
  const cacheKey = useCallback(
    (tabIdx: number) =>
      `${loginid}-${companyCode}-${TAB_STATUS[tabIdx]}`,
    [loginid, companyCode]
  );
  ...
  const initialCacheKey =
    `${loginid}-${companyCode}-${TAB_STATUS[initialTab]}`;

  const [rows, setRows] = useState<Row[]>(
    () => taskPageCache.get(initialCacheKey) ?? []
  );

  const [loading, setLoading] = useState(
    () => !taskPageCache.has(initialCacheKey)
  );
  ...
  const [activeWeightage, setActiveWeightage] =
    useState<ActiveWeightage | null>(null);
  ...
  useEffect(() => {
    ...
    pamsSelect({
      parameter: "appraisal_weightage_list",
      loginid,
      code1: companyCode,
    })
      .then((data) => {
        ...
        const active = wRows?.find(
          (r) => String(r.IS_ACTIVE) === "Y"
        );
  ...
  useEffect(() => {
    const loadLookups = async () => {
      try {
        ...
        setPeriods(
          periodsData.map(normalizeRow)
        );

        setEmployees(
          employeesData.map(normalizeRow)
        );
      } catch (error) {
        console.error(
  ...
  const fetchData = useCallback(
    async (
      ...
      try {
        ...
        if (
          myRequestId !== fetchRequestId.current
        ) {
          return;
        }
  ...
  useEffect(() => {
    const hasCached = taskPageCache.has(
      cacheKey(activeTab)
    );
  ...
  const openAppraisalTabsPage = (
    ...
    navigate(
      `/workspace/pams/appraisal/view/${docNo}` +
  ...
  const saveRecord = async () => {
    ...
    if (
      ...
      setNotice({
        type: "error",
        message:
          "Employee and period are required",
      });
    ...
    try {
      await pamsSave({
        ...
        val1s5:
          formData.APPRAISAL_DOC_NO,

        val1s6:
          formData.APPRAISAL_DOC_DATE,

        val1s7:
          formData.APPRAISAL_FROM,

        val1s8:
          formData.APPRAISAL_TO,
  ...
  const handleDelete = async (
    ...
    try {
      await pamsDelete({
        parameter:
          "delete_appraisal_hdr",

        loginid,

        code1:
          text(
            row.APPRAISAL_DOC_NO
          ),
  ...
  const handleBulkApprove = async () => {
    ...
    if (!ids.length) {
      setNotice({
        type: "warning",
        message:
          "Please select at least one appraisal!",
      });
    ...
    try {
      await pamsSelect({
        parameter:
          "proc_update_pams_doc_status_bulk",
      ...
      setNotice({
        type: "success",
        message:
          "Appraisals approved successfully!",
      });
  ...
  const handleNotifyNextLevel = async (
    flowLevel: number,
    periodNumber: string
  ) => {
    const key =
      `${flowLevel}-${periodNumber}`;
    ...
    try {
      ...
      if (result === "SUCCESS") {
        toast.success(
          `Period ${periodNumber} has been sent to the next level for review.`
        );
      } else if (
        ...
        toast.warning(
          `${pendingCount} employee appraisal(s) are still pending your review. Please complete all reviews before notifying the next level.`
        );
      } else if (
        result === "NOTHING_TO_NOTIFY"
      ) {
        toast.warning(
          "There are no completed appraisals ready to notify at this time."
        );
      } else {
        toast.error(
          result ||
          "Something went wrong while notifying the next level."
        );
      }
      ...
      toast.error(
        err instanceof Error
          ? err.message
          : "Failed to notify the next level. Please try again."
      );
    } finally {
      setNotifying(null);
    }
  ...
  >(() => {
    ...
    cols.push({
      ...
      cell: ({ row }) => {
        const desgCode =
          text(
            row.original.DESG_CODE
          );

        const desgName =
          text(
            row.original.DESG_NAME
          );
  ...
  return (
    ...
    >
      ...
      {!loading &&
        hodBatches
          ...
          .map(
            (batch) => {
              const key =
                `${HOD_READY_FLOW_LEVEL}-${batch.PERIOD_NUMBER}`;
              ...
              return (
                ...
                >
                  ...
                  <Button
                    ...
                    onClick={() =>
                      handleNotifyNextLevel(
                        HOD_READY_FLOW_LEVEL,
                        batch.PERIOD_NUMBER
                      )
                    }
                    style={{
      ...
      {/* IMPORTANT:
          DataTable is ALWAYS rendered.
          Do NOT put it inside {loading ? ... : ...}.
          DataTable itself handles the loading UI.
      */}
      <DataTable
        columns={columns}
        data={rows}
