...
export function PurchaseOrderEditor({
    ...
    (async () => {
      try {
        const rows = await getDynamicLookup({
          parameter: "POORDER_ENTRY_FUN_CHECK_GLOBAL_APPR_LEVEL",
  ...
  const handleSaveAsDraft = () =>
    runAction("draft", async () => {
      await runWorkflow("SAVEASDRAFT", form, rows, user?.company_code, user?.loginid || user?.username);
  ...
  const handleSubmit = () => {
    ...
    return runAction("submit", async () => {
      await runWorkflow("SUBMITTED", form, rows, user?.company_code, user?.loginid || user?.username);
    }, editMode ? "Purchase order updated successfully" : "Purchase order created successfully");
  };

  const handleCancel = () =>
    runAction("cancel", async () => {
      await runWorkflow("CANCELED", form, rows, user?.company_code, user?.loginid || user?.username);
  ...
  const confirmReject = () => {
    ...
    return runAction("reject", async () => {
      ...
      await runWorkflow("REJECTED", payloadForm, rows, user?.company_code, user?.loginid || user?.username);
      setRejectDialogOpen(false);
  ...
  const openSendBackDialog = async () => {
    ...
    try {
      const rows = await getDynamicLookup({
        parameter: "POORDER_ENTRY_SENTBACK_USER_LIST",
        code1: user?.company_code,
  ...
  const confirmSendBack = () => {
    ...
    return runAction("sendBack", async () => {
      ...
      await runWorkflow("SENTBACK", payloadForm, rows, user?.company_code, user?.loginid || user?.username);
      setSendBackDialogOpen(false);
