...
const DELETE_PARAMETER = "ERP_CAM_EXPENSE_MASTER_DELETE";
...
// EXPENSE_CODE is the natural/business key for this master (entered or
// looked up by the user), so — unlike an autogenerated doc_no — it's safe
// and expected to sort on directly. Sorting alphabetically keeps the list
// stable and predictable for a lookup-style master table.
const sortByExpenseCode = (rows: ExpenseMasterRow[]): ExpenseMasterRow[] =>
  [...rows].sort((a, b) =>
    String(a.expense_code ?? "").localeCompare(String(b.expense_code ?? "")),
  );
...
export function ExpenseMasterPage() {
  ...
  const loadRows = useCallback(async () => {
    ...
    try {
      ...
      setRows(sortByExpenseCode(list));
    } catch (error) {
      setNotice({
        type: "error",
