...
const Purchase_Request_page = ({ initialTab = 0 }: PurchaseRequestPageProps) => {
  ...
  // ── Fetch (VW_PR_PAGE view — PR records only, CANCEL_FLAG IS NULL handled server-side) ─
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ["purchase-request-page", loginid, companyCode],
    queryFn: () =>
      almsCommonSelect<TPurchaseSummaryTxn>({
        parameter: "Amlspf_VW_PR_PAGE",
        loginid,
        code1: companyCode,
        code2: "", // blank = fetch all non-cancelled PR records for this company; backend should NVL/skip filter when blank
      }),
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "16px", padding: "16px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "13px", color: "#6b7280" }}>
        ...
        <span style={{ color: "#d1d5db" }}>/</span>
        <span style={{ color: "#111827", fontWeight: 500 }}>Purchase Request</span>
      </div>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "12px" }}>
        ...
        <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
          <Button onClick={openAddPopup} style={{ background: "#082A89" }}>
            <Plus size={15} /> Add PR
          </Button>
        </div>
      </div>
